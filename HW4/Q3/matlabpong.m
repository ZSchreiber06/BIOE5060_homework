...
% Last Modified by GUIDE v2.5 03-Dec-2024 15:35:28
...
period_val = hObject.Value;
uidata = get(handles.fmMain, 'userdata');
get(uidata.timer, 'period')
get(handles.sbSpeed, 'value')
set(uidata.timer.Period, 'period', period*2)
set(handles.sbSpeed, 'value', period_va)
end 
...
get(timerdata.pong, {'xdata', 'ydata'}); % Get position of ball 
yred = get(timerdata.redbat, 'ydata');
yblue = get(timerdata.bluebat, 'ydata');
xdata = get(timerdata.pong, 'xdata');
ydata = get(timerdata.pong, 'ydata');


if xdata < 0 
    if yred(1) <= ydata & ydata <= yred(2)
        timerdata.dir(1) = -timerdata.dir(1);
        set(timerdata.pong, 'xdata', xdata + timerdata.dir(1)*0.02);
        set(timerdata.pong, 'ydata', ydata + timerdata.dir(2)*0.02);  
    else 
        set(timerdata.pong, 'xdata', 0.5);
        set(timerdata.pong, 'ydata', 0.5);
        bluescore = str2num(get(timerdata.txBlueScore, 'string'))+1;
        set(timerdata.txRedScore, 'string', num2str(bluescore))
        
    end 
elseif xdata > 1
    if yblue(1) <= ydata & ydata <= yblue(2)
        timerdata.dir(1) = -timerdata.dir(1);
        set(timerdata.pong, 'xdata', xdata + timerdata.dir(1)*0.02);
        set(timerdata.pong, 'ydata', ydata + timerdata.dir(2)*0.02);  
    else 
        set(timerdata.pong, 'xdata', 0.5);
        set(timerdata.pong, 'ydata', 0.5); 
        redscore = str2num(get(timerdata.txRedScore, 'string'))+1;
        set(timerdata.txRedScore, 'string', num2str(redscore))
    end 
elseif ydata < 0 || ydata > 1 
    timerdata.dir(2) = -timerdata.dir(2);
    set(timerdata.pong, 'xdata', xdata + timerdata.dir(1)*0.02);
    set(timerdata.pong, 'ydata', ydata + timerdata.dir(2)*0.02);
end 
set(timerdata.pong, 'xdata', xdata + timerdata.dir(1)*0.02);
set(timerdata.pong, 'ydata', ydata + timerdata.dir(2)*0.02);
set(timerobj, 'userdata', timerdata);  % at the end of the function, save the updated timerdata to timerobj.userdata
end 
...
yblue = get(timerdata.bluebat, 'ydata');
if(eventdata.Key == 'q')
    set(timerdata.redbat, 'ydata', yred+0.01); % move red bar up by 0.01
elseif(eventdata.Key == 'z')
    set(timerdata.redbat, 'ydata', yred-0.01); 
end 
if(eventdata.Key == 'p')
    set(timerdata.bluebat, 'ydata', yblue+0.01);
